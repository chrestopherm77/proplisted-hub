export const CONTRACT_TERMS = `CONTRATO DE PARCERIA COMERCIAL PARA AQUISIÇÃO E USO DE LEADS

Pelo presente instrumento particular, de um lado LEADBAY, pessoa jurídica de direito privado, doravante denominada PLATAFORMA, e, de outro, o PARCEIRO, pessoa física ou jurídica devidamente cadastrada, qualificada e aceita na plataforma, resolvem celebrar o presente Contrato de Parceria Comercial, que se regerá pelas cláusulas e condições abaixo, nos termos da legislação civil e da Lei nº 13.709/2018 (Lei Geral de Proteção de Dados Pessoais – LGPD).


CLÁUSULA 1 — DEFINIÇÕES

Para os fins deste contrato, considera-se:

a) Lead: conjunto limitado de dados pessoais de titular identificado ou identificável, fornecidos voluntariamente mediante consentimento válido, com a finalidade de permitir contato comercial compatível com interesse manifestado;
b) Titular: pessoa natural a quem se referem os dados pessoais;
c) LGPD: Lei nº 13.709/2018;
d) Disponibilização Onerosa de Leads: acesso controlado e limitado a contatos comerciais, no contexto de intermediação, não caracterizando venda ou cessão definitiva de dados pessoais;
e) DPA: Acordo de Tratamento de Dados Pessoais, Anexo I.

CLÁUSULA 2 — OBJETO

2.1. O presente contrato tem por objeto a concessão ao PARCEIRO de acesso à plataforma tecnológica da LEADBAY para aquisição onerosa de leads qualificados, no contexto de intermediação comercial, mediante consentimento válido dos titulares.
2.2. A aquisição de leads não implica garantia de conversão, exclusividade, resultado comercial ou fechamento de negócios.

2.3. O PARCEIRO reconhece que o lead consiste exclusivamente em manifestação de interesse do titular, não havendo garantia de veracidade, atualidade ou intenção de contratação.

CLÁUSULA 3 — NATUREZA DA OPERAÇÃO (NÃO VENDA DE DADOS)

3.1. As Partes reconhecem que a relação não caracteriza venda, cessão definitiva, locação ou exploração econômica autônoma de dados pessoais.
3.2. A operação limita-se à intermediação de contatos, precedida de consentimento livre, informado, específico e inequívoco do titular.

CLÁUSULA 4 — PAPÉIS E RESPONSABILIDADES (LGPD)

4.1. A LEADBAY atua como Controladora no âmbito da coleta, organização, qualificação e disponibilização limitada dos leads.
4.2. O PARCEIRO, ao receber os dados, passa a atuar como CONTROLADOR INDEPENDENTE, sendo exclusivamente responsável pelo tratamento posterior.
4.3. Inexiste solidariedade, corresponsabilidade, subordinação, grupo econômico, joint venture ou co-controladoria entre as Partes.
4.4. Cada Parte responde exclusivamente pelos tratamentos que realizar.

CLÁUSULA 5 — DECLARAÇÕES E GARANTIAS DO PARCEIRO

O PARCEIRO declara e garante que:

a) Atua em conformidade com a LGPD e demais normas aplicáveis;
b) Possui estrutura mínima de governança e segurança da informação;
c) Utilizará os leads estritamente para a finalidade autorizada;
d) Não realizará práticas abusivas, spam, listas frias ou enriquecimento de base;
e) Assumirá integral responsabilidade por seus atos perante titulares e autoridades.

CLÁUSULA 6 — LIMITAÇÃO DE USO DOS LEADS

6.1. O uso dos dados pessoais limita-se ao contato comercial compatível com o interesse manifestado pelo titular.
6.2. É expressamente vedado:

a) Uso para finalidades diversas;
b) Compartilhamento, cessão, revenda ou enriquecimento de base própria;
c) Campanhas genéricas ou não individualizadas;
d) Manutenção dos dados após 60 (sessenta) dias, salvo obrigação legal.

CLÁUSULA 7 — LIMITE DE CONTATO E GOVERNANÇA

7.1. Cada lead poderá ser disponibilizado a até 5 (cinco) parceiros comerciais distintos.
7.2. O contato deverá ocorrer no prazo máximo de 60 (sessenta) dias contados da disponibilização.
7.3. Após esse prazo, fica terminantemente proibido qualquer novo contato.

CLÁUSULA 8 — OBRIGAÇÃO DE FEEDBACK 

8.1. O PARCEIRO obriga-se a fornecer feedback estruturado mínimo, incluindo:

a) Tentativa de contato;
b) Status do lead;
c) Evolução ou conversão comercial.

8.2. O feedback constitui condição essencial de governança da plataforma.
8.3. O descumprimento autoriza, a critério exclusivo da LEADBAY:

a) Redução de prioridade;
b) Suspensão de acesso;
c) Rescisão imediata.

CLÁUSULA 9 — CONFIDENCIALIDADE E PROTEÇÃO DAS INFORMAÇÕES

9.1. Para os fins deste Contrato, consideram-se Informações Confidenciais toda e qualquer informação, dado ou material, independentemente do meio ou forma de acesso, incluindo, mas não se limitando a:
a) dados pessoais e dados pessoais sensíveis, nos termos da LGPD;
b) leads, cadastros, contatos, preferências e registros de interação;
c) informações técnicas, operacionais, comerciais, estratégicas, financeiras e contratuais;
d) políticas internas, relatórios, auditorias, documentos de compliance e segurança da informação;
e) quaisquer informações às quais o PARCEIRO tenha acesso em razão deste Contrato.

9.2. O PARCEIRO compromete-se a:
a) utilizar as Informações Confidenciais exclusivamente para a execução deste Contrato;
b) restringir o acesso apenas a colaboradores ou prepostos estritamente necessários, devidamente obrigados por dever de confidencialidade;
c) adotar medidas técnicas e organizacionais aptas a proteger as Informações Confidenciais contra acesso não autorizado, vazamento ou uso indevido;
d) não divulgar, compartilhar, reproduzir ou explorar as Informações Confidenciais para finalidades diversas.

9.3. A obrigação de confidencialidade:
a) permanecerá vigente por prazo indeterminado em relação a dados pessoais, leads e informações protegidas por lei;
b) subsistirá pelo prazo mínimo de 5 (cinco) anos após a rescisão quanto às demais Informações Confidenciais.

9.4. Não serão consideradas Informações Confidenciais aquelas que:
a) sejam ou se tornem públicas sem violação deste Contrato;
b) sejam comprovadamente conhecidas pelo PARCEIRO antes da contratação;
c) devam ser divulgadas por força de lei, ordem judicial ou determinação de autoridade competente, hipótese em que a LEADBAY deverá ser previamente comunicada, sempre que possível.

9.5. O descumprimento desta cláusula sujeitará o infrator:
a) à obrigação de cessar imediatamente a infração;
b) à indenização integral por perdas e danos, diretos e indiretos;
c) à aplicação das penalidades contratuais cabíveis;
d) sem prejuízo da adoção de medidas judiciais de urgência.

CLÁUSULA 10 — INCIDENTES DE SEGURANÇA, NOTIFICAÇÃO E COOPERAÇÃO

10.1. O PARCEIRO deverá comunicar à LEADBAY, imediatamente e no prazo máximo de 48 (quarenta e oito) horas a partir da ciência, qualquer incidente de segurança que possa acarretar risco ou dano relevante aos titulares de dados pessoais, incluindo, mas não se limitando a acesso não autorizado, vazamento, destruição, perda, alteração ou divulgação indevida de dados pessoais.

10.2. A comunicação deverá conter, no mínimo:
 a) descrição detalhada do incidente;
 b) data e hora da ocorrência e da detecção;
 c) categorias e volume aproximado de dados afetados;
 d) medidas técnicas e organizacionais adotadas para contenção e mitigação;
 e) avaliação preliminar dos riscos e impactos aos titulares.

10.3. O PARCEIRO obriga-se a cooperar integralmente com a LEADBAY, comprometendo-se a:
 a) fornecer todas as informações e documentos solicitados;
 b) permitir auditorias técnicas e jurídicas relacionadas ao incidente;
 c) implementar, às suas expensas, as medidas corretivas e preventivas determinadas.

10.4. Caberá exclusivamente à LEADBAY deliberar sobre a necessidade, o conteúdo e o momento da comunicação do incidente aos titulares e à Autoridade Nacional de Proteção de Dados (ANPD), sem prejuízo do dever de apoio técnico do PARCEIRO.

10.5. Todos os custos decorrentes do incidente de segurança, incluindo investigações, comunicações, auditorias, sanções, indenizações e medidas de mitigação, serão suportados integralmente pelo PARCEIRO, quando o incidente decorrer de sua atuação, direta ou indireta.

10.6. O descumprimento das obrigações previstas nesta cláusula constitui infração grave, autorizando a rescisão imediata do contrato, sem prejuízo da aplicação das penalidades e do direito de indenização.

CLÁUSULA 11 — DEFESA, INDENIZAÇÃO E REGRESSO

11.1. O PARCEIRO obriga-se a assumir integral e exclusiva responsabilidade, inclusive financeira, pela defesa administrativa, judicial ou extrajudicial da LEADBAY em qualquer reclamação, investigação, processo, procedimento, ação coletiva ou individual, termo de ajustamento de conduta (TAC), processo administrativo ou demanda regulatória que decorra, direta ou indiretamente, do tratamento de dados pessoais realizado pelo PARCEIRO.

11.2. A obrigação de defesa inclui, sem limitação:
a) custeio integral de honorários advocatícios, periciais e despesas processuais;
b) pagamento de condenações, multas administrativas, sanções, acordos judiciais ou extrajudiciais;
c) ressarcimento imediato de quaisquer valores adiantados pela LEADBAY.

11.3. A LEADBAY poderá, a seu exclusivo critério:
a) indicar ou aprovar os profissionais responsáveis pela defesa;
b) conduzir a defesa de forma autônoma ou conjunta;
c) celebrar acordos, transações ou termos administrativos, sem prejuízo do direito de regresso.

11.4. A responsabilidade do PARCEIRO subsiste independentemente de alegação de culpa concorrente, coautoria ou corresponsabilidade, salvo se houver decisão judicial transitada em julgado reconhecendo culpa exclusiva e direta da LEADBAY, sem qualquer contribuição do PARCEIRO

11.5. O PARCEIRO obriga-se a reembolsar integralmente a LEADBAY no prazo máximo de 10 (dez) dias contados da notificação, sob pena de multa, correção monetária e juros legais.

11.6. As obrigações previstas nesta cláusula sobrevivem à rescisão do contrato por qualquer motivo.

CLÁUSULA 12 — MULTA CONTRATUAL

12.1. O descumprimento das obrigações previstas neste Contrato, no DPA ou na legislação de proteção de dados sujeitará o PARCEIRO ao pagamento de multa contratual não compensatória, graduada conforme a gravidade da infração, sem prejuízo da apuração de perdas e danos.

12.2. As multas poderão ser aplicadas nos seguintes patamares:
a) Infrações leves ou formais: multa de R$ 5.000,00 (cinco mil reais);
b) Infrações médias, incluindo uso indevido dos dados, descumprimento de prazo ou falhas de governança: multa de R$ 25.000,00 (vinte e cinco mil reais);
c) Infrações graves, incluindo vazamento, compartilhamento indevido, revenda, enriquecimento de base ou descumprimento doloso da LGPD: multa de R$ 100.000,00 (cem mil reais) ou valor equivalente ao dobro do benefício econômico obtido, o que for maior.

12.2.1. As Partes reconhecem que os valores estipulados refletem estimativa razoável dos prejuízos potenciais, custos regulatórios e riscos reputacionais envolvidos, não se caracterizando penalidade excessiva.

12.3. A aplicação da multa não afasta:
a) o dever de indenização integral por perdas e danos;
b) a obrigação de cessar imediatamente a infração;
c) a responsabilização administrativa perante a ANPD.

12.4. As Partes reconhecem que os valores ora pactuados são proporcionais à natureza da atividade, aos riscos regulatórios envolvidos e à potencial exposição da LEADBAY a sanções administrativas e judiciais.

CLÁUSULA 13 — ADEQUAÇÃO REGULATÓRIA E ATUALIZAÇÃO NORMATIVA

13.1. As Partes reconhecem que a atividade objeto deste contrato está sujeita à legislação de proteção de dados pessoais e às normas, guias, recomendações e decisões da Autoridade Nacional de Proteção de Dados (ANPD).

13.2. A LEADBAY poderá promover a atualização das cláusulas estritamente necessárias para assegurar a conformidade legal, sem alterar a natureza econômica do contrato, respeitados os princípios da boa-fé e da proporcionalidade.

13.3. As alterações serão comunicadas ao PARCEIRO, que deverá manifestar concordância expressa ou, alternativamente, adequar seus procedimentos, hipótese que será interpretada como aceite tácito, no prazo máximo de 15 (quinze) dias, sob pena de suspensão automática do acesso à plataforma.

13.4. A recusa ou omissão do PARCEIRO autoriza a rescisão imediata do contrato, sem ônus para a LEADBAY.

CLÁUSULA 14 — RESCISÃO IMEDIATA E EFEITOS

14.1. O presente contrato poderá ser rescindido imediata e unilateralmente pela LEADBAY, independentemente de aviso prévio ou indenização, nas seguintes hipóteses:

a) violação à LGPD ou ao DPA;

b) uso indevido, compartilhamento não autorizado ou retenção irregular de dados pessoais;

c) ausência reiterada de feedback obrigatório;

d) reclamações fundadas de titulares ou risco regulatório relevante;

e) descumprimento de obrigações contratuais essenciais.

14.2. A rescisão implica, de forma automática e imediata:

a) cessação de qualquer tratamento de dados pessoais pelo PARCEIRO;

b) obrigação de eliminação ou anonimização integral dos dados, no prazo máximo de 5 (cinco) dias, salvo obrigação legal em sentido diverso;

c) manutenção das obrigações de confidencialidade, responsabilidade, indenização e cooperação regulatória.

14.3. A rescisão não prejudica o direito da LEADBAY de apurar infrações, aplicar penalidades e buscar indenização por perdas e danos.

CLÁUSULA 15 — VIGÊNCIA E SOBREVIVÊNCIA

15.1. O presente contrato vigerá por prazo indeterminado, a partir da data de aceite eletrônico ou assinatura.

15.2. Sobreviverão à rescisão ou extinção do contrato, pelo prazo legal aplicável ou enquanto subsistirem efeitos jurídicos:

a) as cláusulas relativas à proteção de dados pessoais (LGPD e DPA);

b) confidencialidade;

c) responsabilidade, indenização, defesa e direito de regresso;

d) auditoria, cooperação regulatória e eliminação de dados;

e) foro e legislação aplicável.

CLÁUSULA 16 — FORO

16.1. Fica eleito o foro da comarca do domicílio da LEADBAY, com renúncia expressa a qualquer outro, por mais privilegiado que seja, para dirimir quaisquer controvérsias oriundas deste contrato.`;

export const DPA_TERMS = `ACORDO DE TRATAMENTO DE DADOS PESSOAIS
(DATA PROCESSING AGREEMENT – DPA)

Este Acordo de Tratamento de Dados Pessoais ("DPA") é celebrado entre LEADBAY e o PARCEIRO, como Anexo I e parte integrante do Contrato de Parceria Comercial para Aquisição e Uso de Leads, regulando o tratamento de dados pessoais realizado no contexto da intermediação de contatos comerciais, nos termos da Lei nº 13.709/2018 (Lei Geral de Proteção de Dados Pessoais – LGPD).


CLÁUSULA 1 — OBJETO E ESCOPO

1.1. O presente DPA tem por objeto regular as responsabilidades, obrigações, limites e medidas de governança aplicáveis ao tratamento de dados pessoais realizado pelas Partes no contexto do contrato principal.

1.2. Este DPA aplica-se exclusivamente aos dados pessoais disponibilizados pela LEADBAY ao PARCEIRO na forma de leads, não abrangendo dados obtidos por outras fontes ou tratados fora do escopo contratual.


CLÁUSULA 2 — DEFINIÇÕES

2.1. Para fins deste DPA, aplicam-se as definições da LGPD e, adicionalmente:

a) Lead: dados pessoais limitados fornecidos voluntariamente por titular, mediante consentimento válido, com a finalidade de contato comercial específico;
b) Tratamento: toda operação realizada com dados pessoais, nos termos do art. 5º, X, da LGPD;
c) Incidente de Segurança: qualquer evento que resulte em acesso não autorizado, vazamento, perda, destruição ou tratamento indevido de dados pessoais.


CLÁUSULA 3 — PAPÉIS DAS PARTES

3.1. A LEADBAY atua como Controladora de Dados Pessoais exclusivamente no âmbito da coleta, organização, qualificação, armazenamento temporário e disponibilização limitada dos leads.

3.2. O PARCEIRO, a partir do recebimento dos dados, passa a atuar como CONTROLADOR INDEPENDENTE, sendo integralmente responsável pelo tratamento posterior realizado.

3.3. As Partes reconhecem que:

• não há co-controladoria;
• não há solidariedade automática;
• não há grupo econômico, representação ou subordinação.


CLÁUSULA 4 — BASE LEGAL

4.1. O tratamento realizado pela LEADBAY fundamenta-se no consentimento livre, informado, específico e inequívoco do titular, nos termos do art. 7º, I, da LGPD.

4.2. O PARCEIRO compromete-se a tratar os dados exclusivamente dentro do escopo do consentimento obtido, sendo vedada a invocação de base legal diversa para fins de contato comercial, sem novo consentimento válido.


CLÁUSULA 5 — DESCRIÇÃO DOS DADOS TRATADOS

5.1. Os dados pessoais tratados incluem:

• nome;
• telefone;
• e-mail;
• preferências comerciais declaradas voluntariamente pelo titular;
• dados técnicos mínimos de registro da coleta.

5.2. É expressamente vedada a coleta ou tratamento de dados pessoais sensíveis, nos termos do art. 5º, II, da LGPD.


CLÁUSULA 6 — FINALIDADE E LIMITAÇÃO DO TRATAMENTO

6.1. O PARCEIRO compromete-se a tratar os dados pessoais exclusivamente para:

• contato comercial direto;
• apresentação de produtos ou serviços compatíveis com o interesse manifestado.

6.2. É terminantemente proibido:
 a) utilizar os dados para finalidades diversas;
 b) enriquecer bases próprias;
 c) revender, compartilhar ou ceder dados;
 d) utilizar os dados para campanhas genéricas ou listas frias.


CLÁUSULA 7 — PRAZO DE RETENÇÃO E ELIMINAÇÃO

7.1. O tratamento dos dados pessoais pelo PARCEIRO fica limitado ao prazo máximo de 60 (sessenta) dias, contados da disponibilização do lead.

7.2. Decorrido esse prazo, o PARCEIRO deverá:

• eliminar os dados pessoais; ou
• proceder à anonimização irreversível,
salvo obrigação legal que justifique retenção diversa.

7.3. A LEADBAY poderá exigir comprovação documental da eliminação.


CLÁUSULA 8 — SEGURANÇA DA INFORMAÇÃO E MEDIDAS PROPORCIONAIS

8.1. O PARCEIRO compromete-se a adotar medidas técnicas e administrativas compatíveis com sua natureza, porte, estrutura operacional e os riscos envolvidos, nos termos do artigo 46 da Lei nº 13.709/2018 (LGPD), destinadas a proteger os dados pessoais recebidos contra acessos não autorizados, vazamentos, perdas ou uso indevido.

8.2. No caso de PARCEIRO pessoa física ou pessoa jurídica de pequeno porte, tais medidas deverão incluir, no mínimo:
 a) uso de dispositivos pessoais protegidos por senha, biometria ou mecanismo equivalente;
 b) não compartilhamento de dispositivos, contas ou credenciais de acesso;
 c) restrição do acesso aos dados exclusivamente ao próprio PARCEIRO;
 d) utilização de aplicativos e serviços com autenticação nativa e criptografia padrão do fornecedor;
 e) exclusão dos dados pessoais após o prazo máximo autorizado de tratamento.

8.3. O PARCEIRO compromete-se a não armazenar os dados pessoais em ambientes públicos, desprotegidos ou compartilhados, bem como a não transferi-los para terceiros ou sistemas não autorizados.

8.4. O PARCEIRO é integralmente responsável pelos dispositivos, contas, meios de comunicação e ferramentas que utilizar para o tratamento dos dados pessoais recebidos, bem como por quaisquer atos de seus colaboradores, prepostos ou terceiros sob sua responsabilidade.


CLÁUSULA 9 — INCIDENTES DE SEGURANÇA

9.1. O PARCEIRO deverá comunicar à LEADBAY, de forma imediata e no prazo máximo de 48 (quarenta e oito) horas a contar do conhecimento, a ocorrência de qualquer incidente de segurança que possa acarretar risco ou dano relevante aos titulares dos dados pessoais recebidos por meio da plataforma, nos termos do artigo 48 da Lei nº 13.709/2018 (LGPD).

9.2. A comunicação deverá conter, na medida do possível e conforme as informações disponíveis à época, ao menos:
 a) descrição da natureza do incidente;
 b) categoria e volume aproximado de dados pessoais afetados;
 c) medidas técnicas e administrativas adotadas para contenção e mitigação;
 d) avaliação preliminar dos impactos, riscos e potenciais danos aos titulares.

9.3. O PARCEIRO compromete-se a complementar as informações prestadas sempre que houver novos dados relevantes, bem como a cooperar com a LEADBAY na adoção das providências necessárias perante titulares e autoridades competentes.


CLÁUSULA 10 — DIREITOS DOS TITULARES

10.1. O PARCEIRO compromete-se a cooperar de forma diligente para o atendimento dos direitos dos titulares previstos no artigo 18 da Lei nº 13.709/2018 (LGPD), no âmbito dos dados pessoais que tratar na condição de Controlador Independente.

10.2. Sempre que receber solicitação direta de titular relacionada a dados pessoais obtidos por intermédio da plataforma, o PARCEIRO deverá:
a) atender a solicitação dentro dos prazos legais aplicáveis; e
b) comunicar a LEADBAY sobre a existência da solicitação, sempre que esta estiver relacionada a dados compartilhados no âmbito da plataforma ou possa gerar impacto regulatório, judicial ou reputacional à LEADBAY.

10.3. O PARCEIRO compromete-se a não adotar qualquer conduta que possa comprometer ou restringir o exercício dos direitos do titular.


CLÁUSULA 11 — AUDITORIA E COMPROVAÇÃO

11.1. A LEADBAY poderá, a qualquer tempo, mediante solicitação razoável e proporcional à natureza da operação, requerer do PARCEIRO:
 a) declarações formais de conformidade com este DPA e com a LGPD;
 b) evidências documentais ou operacionais de exclusão, anonimização ou interrupção do tratamento dos dados pessoais;
 c) comprovação das medidas mínimas de segurança adotadas, compatíveis com sua estrutura operacional.

11.2. As solicitações previstas nesta cláusula não configuram violação de sigilo comercial, profissional ou estratégico, limitando-se exclusivamente à verificação do cumprimento das obrigações contratuais e legais relativas à proteção de dados pessoais.


CLÁUSULA 12 — SUBCONTRATAÇÃO E TRANSFERÊNCIA DE DADOS

12.1. É expressamente vedada a subcontratação, delegação ou compartilhamento dos dados pessoais recebidos com terceiros para fins de tratamento, salvo mediante autorização prévia, expressa e específica da LEADBAY.

12.2. É vedada a transferência internacional de dados pessoais tratados no âmbito deste contrato, salvo mediante autorização expressa da LEADBAY e observância dos requisitos legais previstos nos artigos 33 e seguintes da LGPD.


CLÁUSULA 13 — RESPONSABILIDADE E DIREITO DE REGRESSO

13.1. Cada Parte responderá, de forma exclusiva e integral, pelos tratamentos de dados pessoais que realizar no âmbito de suas respectivas atividades e competências.

13.2. O PARCEIRO assumirá integral responsabilidade por quaisquer:
 a) reclamações, denúncias ou demandas formuladas por titulares de dados;
 b) sanções, autos de infração ou penalidades administrativas aplicadas por autoridades competentes;
 c) ações judiciais, individuais ou coletivas;
 d) danos materiais, morais, coletivos ou reputacionais decorrentes de seu tratamento de dados pessoais.

13.3. O PARCEIRO obriga-se a ressarcir integralmente a LEADBAY por quaisquer valores despendidos em decorrência dos eventos descritos nesta cláusula, incluindo multas, condenações, acordos, custas processuais e honorários advocatícios, sem prejuízo do exercício do direito de regresso.


CLÁUSULA 14 — VIGÊNCIA E SOBREVIVÊNCIA

14.1. Este DPA permanecerá vigente enquanto houver qualquer tratamento de dados pessoais realizado pelo PARCEIRO em decorrência do contrato principal.

14.2. As obrigações relativas à confidencialidade, segurança da informação, responsabilidade, indenização, auditoria e cooperação regulatória sobreviverão ao término ou rescisão do contrato, pelo prazo necessário ao cumprimento das obrigações legais e à defesa de direitos.


CLÁUSULA 15 — PREVALÊNCIA

15.1. Em caso de conflito ou divergência entre as disposições deste DPA e o contrato principal, prevalecerão as disposições deste DPA no que se refere especificamente às matérias relacionadas à proteção de dados pessoais e à LGPD.

CLÁUSULA 16 — NÃO REPRESENTAÇÃO

O PARCEIRO declara que não atua, em nenhuma hipótese, como representante, preposto, agente ou mandatário da LEADBAY perante os titulares dos dados, comprometendo-se a não utilizar o nome, marca ou identidade da LEADBAY em comunicações com titulares, salvo autorização expressa e específica.`;

export const TERMS_OF_USE = `TERMO DE USO DA PLATAFORMA LEADBAY

Este Termo de Uso ("Termo") regula o acesso, a aquisição e o uso de leads disponibilizados pela LEADBAY, devendo ser aceito eletronicamente pelo PARCEIRO, pessoa física ou jurídica, como condição indispensável para utilização da plataforma.

O aceite eletrônico implica concordância integral, irrevogável e irretratável, nos termos do art. 107 do Código Civil e da Lei nº 13.709/2018 (LGPD).


CLÁUSULA 1 — OBJETO E NATUREZA JURÍDICA

1.1. O presente Termo disciplina as regras para acesso e uso de leads qualificados, consistentes em contatos comerciais obtidos mediante consentimento válido dos titulares, no contexto de intermediação comercial, não caracterizando venda, cessão definitiva ou exploração autônoma de dados pessoais.

1.2. O lead representa mera manifestação de interesse, inexistindo garantia de:

• veracidade;
• atualidade;
• intenção de contratação;
• exclusividade;
• conversão ou resultado econômico.


CLÁUSULA 2 — ACEITE ELETRÔNICO E VINCULAÇÃO

2.1. O aceite eletrônico:

• vincula juridicamente o PARCEIRO;
• possui validade probatória plena;
• produz efeitos equivalentes à assinatura física.

2.2. O PARCEIRO declara ter lido, compreendido e aceito:

• este Termo;
• o Contrato Principal;
• o DPA;
• a Política de Privacidade da LEADBAY.


CLÁUSULA 3 — PAPEL DAS PARTES (LGPD)

3.1. A LEADBAY atua como Controladora exclusivamente na coleta, qualificação e disponibilização dos leads.

3.2. O PARCEIRO, a partir do acesso aos dados, passa a atuar como CONTROLADOR INDEPENDENTE, assumindo integral responsabilidade pelo tratamento posterior.

3.3. Não há:

• co-controladoria;
• solidariedade automática;
• vínculo societário;
• representação;
• subordinação.


CLÁUSULA 4 — LIMITES DE USO DOS LEADS

4.1. O uso dos dados pessoais limita-se estritamente a:

• contato comercial individualizado;
• oferta compatível com o interesse manifestado pelo titular.

4.2. É expressamente vedado:

• enriquecimento de base própria;
• revenda, cessão ou compartilhamento;
• campanhas genéricas ou listas frias;
• uso para finalidade diversa;
• retenção superior a 60 dias.


CLÁUSULA 5 — LIMITE DE COMPARTILHAMENTO E CONTATO

5.1. O PARCEIRO declara ciência de que:

• cada lead pode ser compartilhado com até 5 parceiros distintos;
• o prazo máximo de contato é de 60 dias.

5.2. Decorrido o prazo, é terminantemente proibido qualquer novo contato.


CLÁUSULA 6 — FEEDBACK OBRIGATÓRIO

6.1. O PARCEIRO compromete-se a fornecer feedback mínimo estruturado, incluindo:

• tentativa de contato;
• status do lead;
• conversão ou não conversão.

6.2. O feedback constitui condição essencial de governança da plataforma.

6.3. O descumprimento autoriza:

• suspensão imediata;
• bloqueio de acesso;
• rescisão unilateral.


CLÁUSULA 7 — SEGURANÇA DA INFORMAÇÃO

7.1. O PARCEIRO compromete-se a adotar medidas proporcionais à sua estrutura, nos termos do art. 46 da LGPD.

7.2. No mínimo:

• proteção de dispositivos;
• não compartilhamento de credenciais;
• exclusão dos dados no prazo autorizado.


CLÁUSULA 8 — INCIDENTES E COOPERAÇÃO

8.1. Incidentes de segurança deverão ser comunicados em até 48 horas.

8.2. O PARCEIRO cooperará integralmente com:

• a LEADBAY;
• titulares;
• autoridades.


CLÁUSULA 9 — RESPONSABILIDADE, INDENIZAÇÃO E REGRESSO

9.1. O PARCEIRO assume responsabilidade integral e exclusiva por seu tratamento de dados.

9.2. Obriga-se a:

• defender a LEADBAY;
• indenizar perdas, danos, multas e condenações;
• ressarcir custos jurídicos e regulatórios.


CLÁUSULA 10 — LIMITAÇÃO DE RESPONSABILIDADE DA LEADBAY

10.1. A LEADBAY não se responsabiliza por:

• condutas do PARCEIRO;
• uso indevido dos dados;
• abordagem comercial;
• descumprimento legal pelo PARCEIRO.


CLÁUSULA 11 — SUSPENSÃO E RESCISÃO

11.1. O acesso poderá ser suspenso ou rescindido imediatamente, sem aviso prévio, em caso de:

• violação à LGPD;
• ausência de feedback;
• reclamações reiteradas;
• risco regulatório.


POLÍTICA DE PRIVACIDADE

Esta Política descreve, de forma transparente e detalhada, como a LEADBAY trata dados pessoais no contexto de intermediação de contatos comerciais, em conformidade com a LGPD.


1. QUEM SOMOS E COMO ATUAMOS

A LEADBAY atua como plataforma intermediadora, conectando titulares interessados a parceiros comerciais, mediante consentimento válido.


2. DADOS COLETADOS

• nome;
• telefone;
• e-mail;
• preferências comerciais;
• dados técnicos mínimos de registro.

Não coletamos dados sensíveis.


3. FINALIDADE DO TRATAMENTO

• permitir contato comercial;
• intermediar oportunidades;
• viabilizar oferta compatível com o interesse declarado.


4. COMPARTILHAMENTO DE DADOS

4.1. Seus dados poderão ser compartilhados com até:

• 5 parceiros comerciais distintos.

4.2. Cada parceiro:

• atua como CONTROLADOR INDEPENDENTE;
• responde exclusivamente por seu uso.


5. PRAZO DE USO E RETENÇÃO

• contato limitado a 60 dias;
• após esse prazo, os parceiros devem eliminar os dados.


6. DIREITOS DOS TITULARES

Você pode:

• confirmar tratamento;
• acessar;
• corrigir;
• revogar consentimento;
• solicitar exclusão.


7. LIMITAÇÃO DE RESPONSABILIDADE

A LEADBAY:

• não controla o tratamento posterior feito pelos parceiros;
• não se responsabiliza por condutas individuais após o compartilhamento;
• atua como intermediadora.


8. SEGURANÇA E GOVERNANÇA

Adotamos medidas técnicas e organizacionais adequadas para proteção dos dados enquanto sob nossa responsabilidade.


9. BASE LEGAL

O tratamento ocorre com base no consentimento, nos termos do art. 7º, I, da LGPD.


10. ALTERAÇÕES

Esta Política poderá ser atualizada para refletir:

• alterações normativas;
• orientações da ANPD.


11. CONTATO

Solicitações podem ser feitas pelos canais oficiais da plataforma.`;
