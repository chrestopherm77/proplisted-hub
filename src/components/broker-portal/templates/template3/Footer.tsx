...
export function Footer({ portal, onNav }: { portal: BrokerPortal; onNav: (s: string) => void }) {
  ...
  return (
    <footer className="text-white" style={{ background: 'var(--bp-bg)' }}>
      <div className="container mx-auto px-4 py-10 grid md:grid-cols-4 gap-8">
        <div>
          {b.logo_url
            ? <img src={b.logo_url} alt="logo" className="h-20 brightness-0 invert opacity-90" />
            : <p className="font-serif text-xl">{portal.seo?.title || 'Imobiliária'}</p>}
        </div>
        <div className="text-sm space-y-2">
          <p className="font-semibold">{portal.seo?.title || 'Imobiliária'}</p>
          {b.address && <p className="text-xs text-white/80">{b.address}</p>}
          {b.whatsapp && (
            <a href={`https://wa.me/${String(b.whatsapp).replace(/\D/g, '')}`} className="flex items-center gap-2 mt-2">
              <Phone className="h-4 w-4 text-green-300" /> {b.phone || b.whatsapp}
            </a>
          )}
          {b.email && (
            <button onClick={copyEmail} className="flex items-center gap-1 text-sm">Ver e-mail <Copy className="h-3 w-3" /></button>
          )}
          {(b.broker_name || b.creci) && (
            <div className="pt-2 text-xs text-white/80">
              {b.broker_name && <p className="font-semibold">{b.broker_name}</p>}
              {b.creci && <p>CRECI {b.creci}</p>}
            </div>
          )}
        </div>
