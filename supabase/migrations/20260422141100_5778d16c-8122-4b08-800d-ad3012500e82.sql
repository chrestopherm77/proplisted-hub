-- Add missing columns to user_brands
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint WHERE conname = 'user_brands_user_id_key'
  ) THEN
    ALTER TABLE public.user_brands ADD CONSTRAINT user_brands_user_id_key UNIQUE (user_id);
  END IF;
END$$;

-- Update get_public_property to also return brand fields
